...
export default class DeliverySettingsController {
  static async getDeliverySettings(req, res: Response, _next: NextFunction) {
    try {
      const page = req.query.page || 1;
      const pageSize = req.query.pageSize || 20;

      const deliverySettinngsRepo = getDeliverySettingsRepository();
      const response = await deliverySettinngsRepo.findByPartnerId(req.user.id, page, pageSize);

      const paginatedResponse = new PaginatedResponse(response, page, pageSize);

      util.setSuccess(200, 'successful', null);
      util.setPagination(paginatedResponse);
  ...
  static async createNewDeliverySettings(req, res: Response, _next: NextFunction) {
    try {
      ...
      const payload = pick(req.body, ['lower_bound', 'upper_bound', 'cost']);
  ...
  static async updateDeliverySettings(req, res: Response, _next: NextFunction) {
    try {
      ...
      // This method checks if the lower or upper bound already exist for this partner
      await deliverySettingsRepo.findByIdAndLowerBoundAndPartner(
        deliverySettings.id,
        payload.lower_bound,
        user.id
      );
